...
#ramping_rate = Rmax
K_ratio_min = 1
Rmax_ratio_min = 6/10   # ADOC1 = 0.65, LAMC1 = 0.6, NHGC1 = 0.25, NBBC1 = 0.45, ORDC1 = 0.6
...
"""
# load and plot risk curve
pars = pickle.load(open('data/%s/%s/%s_param-risk-thresholds_tocs-reset=%s_fixed=%s-%s_Krat=%s_Rmaxrat=%s_RRrat=%s_seed-%s.pkl'%(loc,site,syn_samp,tocs_reset,fixed_pool,fixed_pool_value,round(K_ratios[p],2),round(Rmax_ratios[p],2),round(rr_ratios[p],2),seed),'rb'),encoding='latin1')
risk_curve = create_param_risk_curve(pars['lo'],pars['hi'],pars['pwr'],pars['no_risk'],pars['all_risk'])

plt.plot(np.arange(nl+1)[1:],risk_curve)
