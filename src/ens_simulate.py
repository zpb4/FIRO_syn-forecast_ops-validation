...
#pars = pickle.load(open('data/%s/%s/%s_risk-thresholds_tocs-reset=%s_fixed=%s-%s_Krat=%s_Rmaxrat=%s_RRrat=%s_seed-%s.pkl'%(loc,site,opt_samp,tocs_reset,fixed_pool,fixed_pool_value,round(K_ratios[p],2),round(Rmax_ratios[p],2),round(rr_ratios[p],2),seed),'rb'),encoding='latin1')
#risk_curve = syn_util.create_risk_curve(pars['x'])

#Q,Q_MSG,Qf,Qf_MSG,dowy,tocs_inp,df_idx = model.extract(sd,ed,forecast_type='syn',syn_vers='v1',syn_sample='Synth1',gen_path=gen_path,Rsyn_path='../Synthetic-Forecast-v1-FIRO-DISES/',forecast_param='a',cal_type='cal',opt_pcnt='')
Q,Qf,dowy,tocs,df_idx = model.extract(sd,ed,forecast_type=sim_forecast,syn_sample=syn_samp,Rsyn_path=syn_path2,syn_vers=syn_vers2,forecast_param='a',loc=loc,site=site,opt_pcnt=syn_vers2_pct,gen_setup=syn_vers2_setup,K=K_scale)
Qf = Qf[:,:,:max_lds]
ne = np.shape(Qf)[1]
...
"""
#demand calculations assuming baseline policy storage/based releases
dmd_par = pickle.load(open('./data/demand-data.pkl','rb'),encoding='latin1')
if mod_vers[-1] == 'd':
    dmd_t = model.simulate_baseline_demand(Q=Q, Q_MSG=Q_MSG, Qf=Qf, Qf_MSG=Qf_MSG, dowy=dowy, tocs=tocs_inp, dmd_params=dmd_par)[6]
#o/w, do not include demand in simulation
else:
    dmd_t = np.zeros_like(Q)
"""
...
for i in range(nsamps):
    ...
    #version 1
    Q,Qf,dowy,tocs,df_idx = model.extract(sd,ed,forecast_type=sim_forecast,syn_sample=syn_samp,Rsyn_path=syn_path1,syn_vers=syn_vers1,forecast_param=syn_vers1_param,loc=loc,site=site,opt_pcnt='',gen_setup=syn_vers1_setup,K=K_scale)
    Qf = Qf[:,:,:max_lds]
    Qf_summed = np.cumsum(Qf, axis=2)
    Qf_summed_sorted = np.sort(Qf_summed, axis = 1)
    ix = ((1 - risk_curve) * (ne)).astype(np.int32)-1
    ...
    #version 2
    Q,Qf,dowy,tocs,df_idx = model.extract(sd,ed,forecast_type=sim_forecast,syn_sample=syn_samp,Rsyn_path=syn_path2,syn_vers=syn_vers2,forecast_param='',loc=loc,site=site,opt_pcnt=syn_vers2_pct,gen_setup=syn_vers2_setup,K=K_scale)
    Qf = Qf[:,:,:max_lds]
    Qf_summed = np.cumsum(Qf, axis=2)
    Qf_summed_sorted = np.sort(Qf_summed, axis = 1)
    ix = ((1 - risk_curve) * (ne)).astype(np.int32)-1
...
np.savez_compressed('out/%s/%s/sim-array_synforc-%s_Krat=%s_Rmaxrat=%s_RRrat=%s_seed-%s.npz' %(loc,site,syn_vers1+svers,round(K_ratios[p],2),round(Rmax_ratios[p],2),round(rr_ratios[p],2),seed), arr=comb_arrayv1)
np.savez_compressed('out/%s/%s/sim-array_synforc-%s_Krat=%s_Rmaxrat=%s_RRrat=%s_seed-%s.npz' %(loc,site,syn_vers2+svers,round(K_ratios[p],2),round(Rmax_ratios[p],2),round(rr_ratios[p],2),seed), arr=comb_arrayv2)
